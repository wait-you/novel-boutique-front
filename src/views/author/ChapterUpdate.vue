        ...
        <form method="post" action="./register.html" id="form2">
          <div class="user_l">
            ...
            <ul class="log_list">
              ...
              <b>章节名：</b>
              <li><input type="text" v-model="chapterInfo.chapterName" id="bookIndex" name="bookIndex" class="s_input" ></li>
              <b>章节内容：</b>
              <li id="contentLi">
                <textarea v-model="chapterInfo.chapterContent"  name="bookContent" rows="30" cols="80" id="bookContent" class="textarea"></textarea>
              </li>
              <b>是否收费：</b>
              <li>
                <input type="radio" name="isVip" value="0" v-model="chapterInfo.isVip">免费
                <input type="radio" name="isVip" value="1" v-model="chapterInfo.isVip">收费</li><br/>
              <li style="margin-top: 10px">
                <input type="button" @click="updateChapterById" name="btnRegister" value="提交" id="btnRegister" class="btn_red">
              </li>
            </ul>
          </div>
        </form>
...
import {reactive, toRefs, onMounted, ref, onActivated} from "vue";
import { useRouter, useRoute } from "vue-router";
import {updateChapter, deleteChapter, getChapterInfo} from "@/api/author";
import AuthorHeader from "@/components/author/Header.vue";
import LogList from "@/components/author/LogList";
...
export default {
  ...
  setup() {
    ...
    const load = async () => {
      const { data } = await getChapterInfo(state.chapterId);
      state.chapterInfo.chapterName = data.chapterName;
      state.chapterInfo.chapterContent = data.chapterContent;
      state.chapterInfo.isVip = data.isVip;
    };

    const updateChapterById = () => {
      updateChapter(state.chapterInfo).then(res => {
        if (res.code === '00000'){
