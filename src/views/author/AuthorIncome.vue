<template>
  <div class="my_r">
    <div class="my_bookshelf">
      <div class="title cf">
        <h2 class="fl ml10"><router-link active-class="red" :to="{name: 'authorIncomeDetail'}">订阅明细</router-link></h2><i
          class="fl ml20 mr20 font16">|</i>
        <h2 class="fl"><router-link :to="{name: 'authorIncome'}" active-class="red">稿费汇总</router-link></h2>
      </div>
      ...
      <div id="divData" class="updateTable">
        <table cellpadding="0" cellspacing="0">
          ...
          <tbody id="bookList">
          <tr class="book_list" v-for="item in authorIncomes">
            <td class="goread">
              {{item.incomeMonth}}
            </td>
            <td class="goread">"
              {{item.preTaxIncome/100}}
            </td>
            <td class="goread">
              {{item.afterTaxIncome/100}}
            </td>
            <td class="goread">{{(item.payStatus == 1 ? '已支付' : '待支付')}}
            </td>
          </tr>
          </tbody>
        </table>
...
import {incomeMonth} from "@/api/author";
...
export default {
  ...
  setup() {
    ...
    const load = async () => {
      const {data} = await incomeMonth(state.page, state.limit);
      state.authorIncomes = data.list;
