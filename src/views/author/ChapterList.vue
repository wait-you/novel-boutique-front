        ...
        <table cellpadding="0" cellspacing="0">
          <thead>
            <tr >
              ...
              <th class="goread">章节名</th>
              <th class="goread">字数</th>
          ...
          <tbody id="bookList">
            <tr class="book_list" vals="291" v-for="(item, index) in chapters"
              :key="index">
              <td class="goread">{{item.chapterNum}}</td>
              <td id="name1358314029098041344" class="goread">
                {{item.chapterName}}
              </td>
              <td class="goread">{{item.chapterWordCount}}</td>
              ...
              <td class="goread" id="opt1358314029098041344">
                <router-link class="redBtn" :to="{name: 'authorChapterUpdate', query: {id: item.id}}">修改 </router-link>
                &nbsp;
                <a @click="deleteChapterById(item.id)" href='javascript:void(0)'>删除 </a>
              </td>
            </tr>
...
export default {
  ...
  setup() {
    ...
    const load = async () => {
      const { data } = await listChapters(state.bookId,state.pageNum, state.pageSize);
