          ...
          <div class="btns" id="optBtn">
            ...
            <span id="cFavs">
              <a href="javascript:void(0);"
                ...
                @click="addBookshelf(book.id, book.firstChapterId)">
                {{ bookshelfDesc }}
              </a>
            </span>
          </div>
...
import {addToBookShelf, comment, deleteComment, updateComment, getBookshelfStatus, removeFromShelf} from "@/api/user";
...
export default {
  ...
  setup() {
    ...
    const addBookshelf = async (bookId, chapterId) => {
      if (state.bookShelfStatus === '0'){
        await addToBookShelf(bookId, chapterId)
        checkBookshelfStatus(bookId)
