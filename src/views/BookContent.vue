          ...
          >
            <ul>
              ...
              <li class="li_shelf" id="cFavs"
                  v-if="bookShelfStatus === '0'">
                <a
                  class="ico_shelf"
                  href="javascript:void(0);"
                  title="加入书架"
                  @click="addFavorites(data.bookInfo.id);"
                  ><b>加书架</b></a>
              </li>
              <li class="li_shelfed" v-else>
                <a class="ico_shelfed" @click="removeFavorites(data.bookInfo.id)" href="javascript:void(0);" title="已收藏"
                  ><b>已收藏</b></a>
              </li>
...
import {addReadHistory, addToBookShelf, getBookshelfStatus, removeFromShelf} from "@/api/user";
export default {
  name: "bookContent",
  components: {
    Top,
  ...
  setup() {
    ...
    onMounted(() => {
      ...
      checkBookshelfStatus(route.params.id)
      addReadHistory(route.params.id, route.params.chapterId)
    ...
    const addFavorites = async (bookId) => {
      await addToBookShelf(bookId, route.params.chapterId)
      ElMessage.success("加入书架成功")
      checkBookshelfStatus(bookId)
    }

    const removeFavorites = async (bookId) => {
